...
result is reasonable: a highly specialised function can (and should) outperform two
(relatively fast) universal converters.  `anytime()` is still compelling as it easier
to use than `as.POSIXct()` by not requiring a format string.

```{r benchmark_speed, cache=TRUE}
library(rbenchmark)
library(fasttime)
inp <- rep("2019-01-02 03:04:05", 100)
v <- benchmark(fasttime=fastPOSIXct(inp),
...
library(parsedate)
inp <- rep("2019-01-02 03:04:05", 100)
v <- benchmark(parsedate=parse_date(inp),
               baseR=as.POSIXct(inp),
               anytime=anytime(inp),
               replications=5e3)
v[,1:4]
